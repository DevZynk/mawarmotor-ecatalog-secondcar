  ...
  hooks: {
    ...
    afterChange: [
      async ({ doc, previousDoc, req, context }) => {
        ...
        const currentBrand = getRelationId(doc.carBrand)
        const previousBrand = getRelationId(previousDoc?.carBrand)

        await updateCount('carBrands', 'carBrand', currentBrand)

        if (previousBrand && previousBrand !== currentBrand) {
          await updateCount('carBrands', 'carBrand', previousBrand)
        }
        ...
        const currentType = getRelationId(doc.carType)
        const previousType = getRelationId(previousDoc?.carType)

        await updateCount('carTypes', 'carType', currentType)

        if (previousType && previousType !== currentType) {
          await updateCount('carTypes', 'carType', previousType)
        }
      },
    ],
    afterDelete: [
      async ({ doc, req }) => {
        ...
        const brandId = typeof doc.carBrand === 'object' ? doc.carBrand?.id : doc.carBrand
        ...
        const typeId = typeof doc.carType === 'object' ? doc.carType?.id : doc.carType

        if (typeId) {
          const typeCount = await payload.count({
            collection: 'cars',
            where: {
              carType: {
                equals: typeId,
